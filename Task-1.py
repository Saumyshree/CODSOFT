...
def open_tasks():
    try:
        tasks = pickle.load(open("task.dg", "rb"))
...
def save_tasks():
    ...
    pickle.dump(tasks, open("task.dg", "wb"))
...
button_add_task = tkinter.Button(root, text="Add Task", bg ='brown', fg='white',font=('TimesNewRoman'), width=48, command=add_task)
button_add_task.pack()

button_delete_task = tkinter.Button(root, text="Delete  Task", bg ='blue',fg='white',font=('TimesNewRoman'), width=48, command=delete_task)
