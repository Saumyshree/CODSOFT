...
def generate_password_and_show(length_entry, complexity_var):
    try:
        ...
        if password:
            password_label.config(text=f"Generated Password: {password}",bg='yellow',fg='black',font=('CourierNewBaltic',16))
        
    except ValueError:
        messagebox.showwarning("Invalid input", "Please enter a valid length.")
root = tk.Tk()
root.title("Password Generator: Saumya")
...
generate_button = tk.Button(root, text="Click to Generate Password", bg='red',fg='white', command=lambda: generate_password_and_show(length_entry, complexity_var))
